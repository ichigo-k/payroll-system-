...
export default async function EditEmployeePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!actor)
    return <NoPermission title="You can’t edit employees" description="Only administrators can change employee records. Pay is set up on the Pay tab by payroll preparers." />
    ...
    <div className="mx-auto max-w-5xl">
      <PageHeader
        breadcrumbs={[
          ...
          { label: name, href: `/portal/financial/employees/${id}` },
          { label: 'Edit' },
        ]}
        title={`Edit ${name}`}
