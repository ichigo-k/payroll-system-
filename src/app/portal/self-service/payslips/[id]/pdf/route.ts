...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const detail = await prisma.payrollDetail.findFirst({
    ...
    select: { payrollRunId: true, employeeCode: true },
  })
  ...
  const pdf = await renderPdf(
    createElement(PayslipsDocument, { meta: { company: data.company, generatedBy: actorName(user), generatedAt: new Date(), draft: false }, run: data.info, lines }),
  )
