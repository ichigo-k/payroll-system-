...
import type { PayrollCalculation, TaxBracket } from './types'
 ...
 */
export function calculateSSNIT(grossIncome: number, employeeRate: number, employerRate: number): { employee: number; employer: number } {
  return {
...
export function calculatePayroll(
  ...
  // Calculate SSNIT
  const { employee: ssnitEmployee, employer: ssnitEmployer } = calculateSSNIT(grossIncome, ssnitEmployeeRate, ssnitEmployerRate)
 ...
 */
export function validateEmployeeData(data: { firstName?: string; lastName?: string; email?: string; employeeId?: string; startDate?: Date }): { valid: boolean; errors: string[] } {
  const errors: string[] = []
...
export function hasPermission(userRole: string, action: string): boolean {
  const permissions: Record<string, string[]> = {
    ...
    PREPARER: ['create_employee', 'edit_employee', 'configure_salary', 'configure_tax', 'submit_payroll', 'view_payroll', 'generate_report'],
    APPROVER: ['view_payroll', 'approve_payroll', 'reject_payroll', 'view_report'],
